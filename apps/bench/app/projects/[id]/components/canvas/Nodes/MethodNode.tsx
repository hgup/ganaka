...
export function MethodNode({ id, data, selected }: NodeProps<MethodNodeType>) {
  ...
  const runMethodNode = useCanvasStore((state) => state.runMethodNode);
  ...
  const handleRun = () => {
    // Assuming you pass down or extract the projectId from context/URL
    runMethodNode(id, projectId);
  };
    ...
    <>
      <Card
        className={`w-64 shadow-xl transition-all duration-200  bg-linear-to-b from-amber-50 to-white dark:from-neutral-800 border dark:to-neutral-800/90 ${
          selected
            ? "dark:border-amber-200 border-amber-500"
            : "border-slate-700"
        }`}
      >
        <Handle
        ...
        {data.methodType === "none" ? (
          ...
          <>
            ...
            <CardContent className="px-4 py-4">
              <div className="flex items-end justify-between">
                <div>
                  ...
                  <div className="text-2xl  text-amber-900 dark:text-amber-100 tracking-tight">
                    $
                    {data.results?.totalIbnr?.toLocaleString("en-US", {
            ...
            <CardFooter>
              ...
              >
                {data.isCalculating ? "Running..." : "Run"}
              </Button>
            </CardFooter>
          </>
        )}
